...
class FeatureBuilder:
    ...
    def build_features(
        ...
        # Confirmed lineups (time-aware: may be absent for far-future fixtures)
        lineup_features = self._extract_lineup_features(lineups, home_id, away_id)
        ...
        home_match_stats = self._extract_recent_match_stats(
            home_recent_stats, home_last_10, home_id, "home"
        )
        away_match_stats = self._extract_recent_match_stats(
            away_recent_stats, away_last_10, away_id, "away"
        )
    ...
    def _extract_lineup_features(self, lineups_response, home_id: int, away_id: int):
        """Extract lightweight features from confirmed lineups.

        We intentionally keep this low-dimensional and robust because lineups
        may be missing until close to kickoff.
        """
        ...
        try:
            entries = lineups_response.get("response") or []
            for entry in entries:
                ...
                subs_count = len(substitutes)

                if team_id == home_id:
                    features["home_lineup_available"] = 1
                    features["home_starting_xi_count"] = starter_count
                    features["home_subs_count"] = subs_count
                    features["home_lineup_confirmed"] = 1 if starter_count >= 11 else 0
                    features["home_lineup_formation_code"] = formation_map.get(formation, 0)
                elif team_id == away_id:
                    features["away_lineup_available"] = 1
                    features["away_starting_xi_count"] = starter_count
                    features["away_subs_count"] = subs_count
                    features["away_lineup_confirmed"] = 1 if starter_count >= 11 else 0
                    features["away_lineup_formation_code"] = formation_map.get(formation, 0)
        except Exception as e:
            print(f"Warning: Failed to extract lineup features: {e}")

        return features

    def _get_team_standing(self, standings_response, team_id):
        """Extract team's league position and points"""
        # Default to a neutral mid-table position with 0 points when standings are missing.
        default_rank, default_points = 10, 0

        if not standings_response or "response" not in standings_response:
            return default_rank, default_points

        try:
            response_entries = standings_response.get("response") or []
            for entry in response_entries:
                league = (entry or {}).get("league") or {}
                standings_groups = league.get("standings") or []

                # API-Football usually returns standings as a list of groups (each group is a list of rows).
                # Some leagues/competitions can have different shapes; handle defensively.
                for group in standings_groups:
                    rows = group if isinstance(group, list) else []
                    for row in rows:
                        row_team_id = ((row or {}).get("team") or {}).get("id")
                        if row_team_id == team_id:
                            ...
                            return (
                                (
                                    int(rank)
                                    if isinstance(rank, (int, float, str)) and str(rank).isdigit()
                                    else default_rank
                                ),
                                (
                                    int(points)
                                    if isinstance(points, (int, float, str))
                                    and str(points).lstrip("-").isdigit()
                                    else default_points
                                ),
                            )
        except Exception:
            # Standings can legitimately be missing or have unexpected structure;
            # don't spam logs on best-effort enrichment.
            return default_rank, default_points
    ...
    def _analyze_form(self, fixtures_response, team_id):
        ...
        form = {
            ...
            "goal_diff": 0,
            "wins_last5": 0,
            "points_last5": 0,  # Added for last 5 matches
        }
    ...
    def _extract_season_stats(self, stats_response):
        ...
        default = {
            ...
            "clean_sheets": 0,
            "xg_for_avg": None,
            "xg_against_avg": None,  # Expected goals
        }
        ...
        try:
            ...
            if "expected" in goals_against:
                xg_against_avg = self._safe_float(
                    goals_against.get("expected", {}).get("average", {}).get("total")
                )
    ...
    def _extract_odds(self, odds_response):
        """Extract betting odds for all 3 core markets: 1X2, Over/Under 2.5, BTTS"""
        default_odds = {
            # 1X2 Market
            "odds_home_win": 0,
            "odds_draw": 0,
            ...
            "odds_btts_no": 0,
            # Availability flags
            "odds_1x2_available": False,
            "odds_ou25_available": False,
            "odds_btts_available": False,
        ...
        try:
            ...
            # Iterate through bookmakers to find all markets
            for bookmaker in odds_response["response"][0].get("bookmakers", []):
                for bet in bookmaker.get("bets", []):
                    ...
                    # 1X2 / Match Winner
                    if bet_name == "Match Winner" and not default_odds["odds_1x2_available"]:
                        for v in values:
                            if v["value"] == "Home":
                        ...
                        default_odds["odds_1x2_available"] = True
                        default_odds["odds_available"] = True

                    # Over/Under 2.5 Goals
                    elif bet_name == "Goals Over/Under" and not default_odds["odds_ou25_available"]:
                        for v in values:
                            val = str(v.get("value", ""))
                            if val == "Over 2.5":
                                default_odds["odds_over_25"] = self._safe_float(v["odd"])
                            elif val == "Under 2.5":
                                default_odds["odds_under_25"] = self._safe_float(v["odd"])
                        if default_odds["odds_over_25"] > 0 or default_odds["odds_under_25"] > 0:
                            default_odds["odds_ou25_available"] = True
                            default_odds["odds_available"] = True

                    # Both Teams To Score (BTTS)
                    elif bet_name == "Both Teams Score" and not default_odds["odds_btts_available"]:
                        for v in values:
                            val = str(v.get("value", ""))
                            if val == "Yes":
                                default_odds["odds_btts_yes"] = self._safe_float(v["odd"])
                            elif val == "No":
                                default_odds["odds_btts_no"] = self._safe_float(v["odd"])
                        if default_odds["odds_btts_yes"] > 0 or default_odds["odds_btts_no"] > 0:
                            default_odds["odds_btts_available"] = True
                            default_odds["odds_available"] = True

                # Stop once we have all 3 markets
                if (
                    default_odds["odds_1x2_available"]
                    and default_odds["odds_ou25_available"]
                    and default_odds["odds_btts_available"]
                ):
                    break
    ...
    def _extract_player_features(self, players_response, prefix):
        ...
        try:
            ...
            for p in players:
                ...
                if goals > 0:
                    scorers.append({"name": player_info.get("name", "Unknown"), "goals": goals})
                    total_goals += goals

                if assists > 0:
                    assisters.append(
                        {"name": player_info.get("name", "Unknown"), "assists": assists}
                    )
            ...
            if scorers:
                ...
                features[f"{prefix}_top_scorer_dependency"] = scorers[0]["goals"] / max(
                    total_goals, 1
                )
    ...
    def _extract_discipline_features(self, fixtures_response, team_id, prefix):
        ...
        try:
            ...
            if matches_analyzed > 0:
                features[f"{prefix}_cards_per_game"] = (
    ...
    def _extract_goal_timing(self, fixtures_response, team_id, prefix):
        ...
        try:
            ...
            if goals_scored["total"] > 0:
                features[f"{prefix}_early_goals_pct"] = (
                    goals_scored["early"] / goals_scored["total"]
                )
                features[f"{prefix}_first_half_goals_pct"] = (
                    goals_scored["first_half"] / goals_scored["total"]
                )
                features[f"{prefix}_late_goals_pct"] = goals_scored["late"] / goals_scored["total"]

            if goals_conceded["total"] > 0:
                features[f"{prefix}_conceded_late_pct"] = (
                    goals_conceded["late"] / goals_conceded["total"]
                )
    ...
    def _extract_competition_features(
        self, league_id, league_round, competition_info=None, round_info=None
    ):
        """
        ...
        if (
            competition_info.get("two_leg_knockout")
            and features["is_knockout_stage"]
            and not features["is_final"]
        ):
