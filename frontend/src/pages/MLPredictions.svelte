<script>
  ...
  import { generateMLPredictionsSEO } from "../services/seoService.js";
  const seoData = generateMLPredictionsSEO();
    ...
    // All supported leagues (centralized)
    ...
    function getServiceStatusLabel(status) {
        return t(`mlPredictions.status.${status}`);
    }
    ...
    async function loadUpcomingMatches() {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(t("mlPredictions.errors.loadFixtures"));
            }
            ...
            error = t("mlPredictions.errors.loadFixtures");
        } finally {
    ...
    async function getPrediction(match) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.detail || t("mlPredictions.errors.predictionFailed"));
            }
            ...
            if (requestId === predictionRequestToken) {
                ...
                // Show scroll hint on mobile after successful prediction load
...
<svelte:window on:click={handleClickOutside} />
    ...
    <div class="prediction-page page-enter">
        <div class="page-header element-enter">
            <h1>{$_("mlPredictions.title")}</h1>
            <p class="text-slate-400 text-sm mt-1">
                {$_("mlPredictions.subtitle")}
            </p>
                ...
                {#if showLeagueDropdown}
                    ...
                    <div class="league-dropdown" on:click|stopPropagation>
                        ...
                        <div class="league-group">
                            <div class="league-group-title">🏆 {$_("mlPredictions.europeanCompetitions")}</div>
                        ...
                        <!-- Top Leagues -->
                        <div class="league-group">
                            <div class="league-group-title">⭐ {$_("mlPredictions.topLeagues")}</div>
                        ...
                        <!-- Second Division -->
                        <div class="league-group">
                            <div class="league-group-title">📋 {$_("mlPredictions.secondDivision")}</div>
                        ...
                        <!-- Friendlies -->
                        <div class="league-group">
                            <div class="league-group-title">🤝 {$_("mlPredictions.friendlies")}</div>
                            {#each leagues.filter(l => l.tier === 4) as l}
                                <button
                                    type="button"
                                    class="league-option {league === l.id ? 'active' : ''}"
                                    on:click={() => changeLeague(l.id)}
            ...
            <div class="service-status">
                ...
                <span>{$_("mlPredictions.aiService")}: {getServiceStatusLabel(mlApiStatus)}</span>
            </div>
        </div>
    </div>
    ...
    <div class="content-grid element-enter stagger-1">
        ...
        <div class="matches-panel">
            <h2>{currentLeague.emoji} {$_("mlPredictions.leagueFixtures", { values: { league: currentLeague.name } })}</h2>

            {#if loadingMatches}
                <div class="loading-state-small">
                    <div class="spinner-small"></div>
                    <p>{$_("fixtures.loading")}</p>
                </div>
            {:else if matches.length === 0}
                <div class="empty-state">
                    <p>{$_("fixtures.noUpcomingFixtures")}</p>
                </div>
            {:else}
                <div class="matches-list">
                    {#each matches as match}
                        ...
                        <button
                            ...
                            on:touchend|preventDefault={() => getPrediction(match)}
                        >
                            ...
                            {#if match.fixture.status?.short === 'NS'}
                                <div class="match-status">{$_("mlPredictions.notStarted")}</div>
                            {/if}
                        </button>
                    {/each}
                </div>
        ...
        <div class="prediction-panel">
            {#if !selectedMatch}
                <div class="placeholder-state">
                    ...
                    <h3>{$_("mlPredictions.selectMatchTitle")}</h3>
                    <p>
                        {$_("mlPredictions.selectMatchDesc")}
                    </p>
                </div>
            {:else if loading}
                <div class="loading-state">
                    <div class="spinner"></div>
                    <p>{$_("mlPredictions.generatingPrediction")}</p>
                </div>
            {:else if error}
                <div class="error-state">
                    <div class="error-icon">⚠️</div>
                    <h3>{$_("mlPredictions.predictionFailed")}</h3>
                    <p>{error}</p>
                    {#if mlApiStatus === "offline"}
                        <p class="error-hint">
                            {$_("mlPredictions.serviceOfflineHint")}
                        </p>
                    {/if}
                    <button
                        type="button"
                        ...
                        on:touchend|preventDefault={() => getPrediction(selectedMatch)}
                    >
                        {$_("common.retry")}
                    </button>
                </div>
                ...
                <div class="prediction-display">
                    ...
                    {#if analysis}
                        <div class="analysis-section" class:expanded={showAnalysis}>
                            <button type="button" class="analysis-toggle" on:click={() => showAnalysis = !showAnalysis} on:touchend|preventDefault={() => showAnalysis = !showAnalysis}>
                                <h3>
