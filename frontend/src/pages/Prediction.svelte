<script>
    ...
    try {
      ...
      if (data && data.fixture_details) {
        addToHistory({
          ...
          home_logo: data.fixture_details.teams?.home?.logo || null,
          away_logo: data.fixture_details.teams?.away?.logo || null,
  ...
  function exportCSV() {
    if (data && data.prediction && data.fixture_details) {
      const result = exportPredictionsToCSV([
        {
          date: data.fixture_details.fixture.date,
  ...
  function exportPDF() {
    if (data && data.prediction && data.fixture_details) {
      const result = exportPredictionsToPDF([
        {
          date: data.fixture_details.fixture.date,
          home_team: data.fixture_details.teams.home.name,
          away_team: data.fixture_details.teams.away.name,
  ...
  // Result feedback: check if match is finished and whether AI was correct
  $: matchStatus = data?.fixture_details?.fixture?.status?.short || "";
  $: matchFinished = ["FT", "AET", "PEN"].includes(matchStatus);
  ...
  $: resultFeedback = (() => {
    ...
    if (predictedHome >= predictedDraw && predictedHome >= predictedAway)
      aiPrediction = "home";
    else if (predictedAway >= predictedHome && predictedAway >= predictedDraw)
      aiPrediction = "away";
    else aiPrediction = "draw";
...
{:else if data}
  <div class="max-w-5xl mx-auto space-y-6 md:space-y-8 page-enter px-4">
    ...
    >
      {#if exportToast}
        <div class="fixed top-20 right-4 z-50 bg-slate-900/95 border border-accent/30 text-white text-sm px-4 py-3 rounded-xl shadow-xl">
          {exportToast}
        </div>
      {/if}
      <button
      ...
      {#if data?.fixture_details && data?.prediction}
        <SharePrediction
          match={{
            teams: data.fixture_details.teams,
            league: data.fixture_details.league,
          }}
    ...
    >
      ...
      {#if data.fixture_details.league}
        ...
        {@const leagueMeta = getLeague(league.id)}
        {@const isEuropean = leagueMeta?.tier === 0}
        {@const isCup = leagueMeta?.tier === 3}
        ...
        <div class="flex justify-center mb-4">
          ...
          >
            ...
            to={`/predictions?league=${league.id}`}
            class="ml-2 inline-flex items-center gap-1 px-2 py-1 text-xs text-slate-400 hover:text-accent transition-colors"
            title={`All ${league.name} predictions`}
          >
            All predictions →
          </Link>        </div>
      {/if}
      ...
      <div class="max-w-2xl mx-auto">
        ...
        >
          <span class="text-emerald-400"
            >{$_("prediction.homeWin").split(" ")[0]}</span
          >
          <span class="text-slate-400">{$_("prediction.draw")}</span>
          <span class="text-rose-400"
            >{$_("prediction.awayWin").split(" ")[0]}</span
          >
        </div>
        <div
    ...
    >
      ...
      <div class="space-y-4 md:space-y-6">
        ...
        <div class="glass-card p-4 md:p-6">
          ...
          >
            Based on proprietary AI analysis
          </div>
        </div>
        ...
        <div class="glass-card p-4 md:p-6">
          <h3
            class="text-base md:text-lg font-bold mb-4 md:mb-6 flex items-center gap-2"
          >
            <span class="text-accent">📊</span> Markets Summary
          </h3>
          ...
          <div class="grid grid-cols-3 gap-4">
            ...
            <div class="text-center p-3 bg-white/5 rounded-lg">
              ...
              <div class="text-lg font-bold text-accent">
                {#if data.prediction.home_win_prob > data.prediction.draw_prob && data.prediction.home_win_prob > data.prediction.away_win_prob}
                  1
                {:else if data.prediction.away_win_prob > data.prediction.draw_prob}
                  2
                {:else}
                  X
                {/if}
            ...
            <div class="text-center p-3 bg-white/5 rounded-lg">
              ...
              <div
                class="text-lg font-bold {(data.prediction.over25_prob || 0) >
                0.5
                  ? 'text-green-400'
                  : 'text-red-400'}"
              >
                {(data.prediction.over25_prob || 0) > 0.5 ? "Over" : "Under"}
              </div>
              <div class="text-xs text-slate-400 mt-1">
                {(((data.prediction.over25_prob || 0) > 0.5
                  ? data.prediction.over25_prob || 0
                  : 1 - (data.prediction.over25_prob || 0)) *
                  100) |
                  0}%
              </div>
            </div>
            ...
            <div class="text-center p-3 bg-white/5 rounded-lg">
              ...
              <div
                class="text-lg font-bold {(data.prediction.btts_prob || 0) > 0.5
                  ? 'text-green-400'
                  : 'text-red-400'}"
              >
                {(data.prediction.btts_prob || 0) > 0.5 ? "Yes" : "No"}
              </div>
              <div class="text-xs text-slate-400 mt-1">
                {(((data.prediction.btts_prob || 0) > 0.5
                  ? data.prediction.btts_prob || 0
                  : 1 - (data.prediction.btts_prob || 0)) *
                  100) |
                  0}%
              </div>
            </div>
