<script>
  ...
  const seoData = generatePageSEO(
    "Admin Metrics",
    "FixtureCast system metrics and performance dashboard.",
    "/admin/metrics",
  );
  import { onMount } from "svelte";
  import { Link } from "svelte-routing";
  import { BACKEND_API_URL, ML_API_URL } from "../config.js";
  ...
  let loading = true;
  let error = null;
  const BACKEND_API = import.meta.env.VITE_BACKEND_API || BACKEND_API_URL;
  const ML_API = import.meta.env.VITE_ML_API || ML_API_URL;
  ...
  async function loadMetrics() {
    try {
      // Load metrics, backtest history, live predictions, and API usage in parallel
      const [metricsRes, backtestRes, liveRes, usageRes] = await Promise.all([
        fetch(`${BACKEND_API}/api/metrics/summary`).catch(() => null),
        fetch(`${BACKEND_API}/api/metrics/backtest-history?limit=52`).catch(
          () => null,
        ),
        fetch(`${ML_API}/api/metrics/history?limit=50`).catch(() => null),
        fetch(`${BACKEND_API}/api/usage`).catch(() => null),
      ]);
      ...
      if (backtestRes?.ok) {
        ...
        backtestHistory = dedupeBacktestHistoryByDate(
          backtestData.history || [],
        );
  ...
  {#if !isAuthenticated}
    ...
    <div class="glass-card p-8 max-w-md mx-auto mt-20">
      <div class="text-center mb-6">
        ...
        <h1 class="text-2xl font-bold mt-4">{$_('adminMetrics.accessRequired')}</h1>
        <p class="text-slate-400 mt-2">{$_('adminMetrics.enterPassword')}</p>
        <p class="text-xs text-amber-400/90 mt-3">
          Note: this password check runs in the browser and is not secure. Use
          it only as a light barrier.
        </p>
      </div>
      ...
      <div class="space-y-4">
        <input
          ...
          placeholder={$_('adminMetrics.enterPasswordPlaceholder')}
  ...
  {:else}
    ...
    <div class="glass-card p-6 md:p-8">
      <div class="flex justify-between items-center mb-4">
        ...
        <Link
          to="/"
          class="px-4 py-2 bg-accent text-white rounded-lg font-medium hover:bg-accent/90 transition"
        >
          ← Back
        </Link>
      </div>
      ...
      <div class="flex gap-2 mt-4 border-b border-white/10 pb-0">
        <button
          ...
          class="px-4 py-2 font-medium transition -mb-px {activeTab ===
          'summary'
        ...
        <button
          ...
          class="px-4 py-2 font-medium transition -mb-px {activeTab === 'live'
            ? 'text-accent border-b-2 border-accent'
            : 'text-slate-400 hover:text-white'}"
        >
          🔴 Live Predictions
    ...
    {#if loading}
      <div class="glass-card p-8 text-center">
        ...
        <p class="mt-3 text-slate-400">{$_('adminMetrics.loading')}</p>
      </div>
    {:else if error}
      <div class="glass-card p-6 border border-red-500/30 bg-red-500/5">
        <p class="text-red-400">{$_('adminMetrics.errorPrefix')} {error}</p>
    ...
    {:else}
      ...
      {#if activeTab === "summary" && metrics}
        <!-- API Usage Monitor -->
        {#if apiUsage}
          {@const usagePercent = (apiUsage.used / apiUsage.limit) * 100}
          {@const isWarning = usagePercent > 70}
          {@const isCritical = usagePercent > 90}
          <div
            class="glass-card p-4 mb-4 {isCritical
              ...
              <h3 class="text-lg font-bold flex items-center gap-2">
                ...
                {:else if isWarning}
                  <span
                    ...
                    >{$_('adminMetrics.warning')}</span
                  >
                {/if}
              </h3>
              <div class="text-sm text-slate-400">
                Resets in {apiUsage.reset_hours}h {apiUsage.reset_minutes}m
              </div>
            </div>
            ...
            <div class="flex items-center gap-4">
              <div class="flex-1">
                <div class="h-4 bg-slate-700 rounded-full overflow-hidden">
                  <div
                    class="h-full transition-all duration-500 {isCritical
                      ? 'bg-red-500'
                      : isWarning
                        ? 'bg-yellow-500'
                        : 'bg-green-500'}"
              ...
              <div class="text-right min-w-[120px]">
                <span
                  class="text-xl font-bold {isCritical
                    ? 'text-red-400'
                    : isWarning
                      ? 'text-yellow-400'
                      : 'text-green-400'}"
                ...
                <span class="text-slate-400">
                  / {apiUsage.limit.toLocaleString()}</span
                >
              </div>
            </div>

            <div class="mt-2 text-xs text-slate-500">
              {usagePercent.toFixed(1)}% used • {apiUsage.remaining.toLocaleString()}
              remaining
            </div>
          </div>
        {/if}
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div class="glass-card p-4">
            <div class="text-slate-400 text-sm font-medium mb-2">
              7-Day Accuracy
            </div>
            <div
              class={`text-3xl font-bold ${getAccuracyColor(metrics["7_day"].accuracy)}`}
            >
              {(metrics["7_day"].accuracy * 100).toFixed(1)}%
            </div>
            <div class="text-xs text-slate-500 mt-2">
              {metrics["7_day"].correct_predictions} / {metrics["7_day"]
          ...
          <div class="glass-card p-4">
            ...
            <div class="text-xs text-slate-500 mt-2">
              Range: {(metrics["7_day"].min_confidence * 100).toFixed(0)}%–{(
                metrics["7_day"].max_confidence * 100
              ).toFixed(0)}%
            </div>
          </div>
          ...
          <div class="glass-card p-4">
            ...
            <div
              class={`text-3xl font-bold ${getCalibrationColor(metrics["7_day"].avg_calibration_error)}`}
            >
          ...
          <div class="glass-card p-4">
            ...
            <div
              class={`text-3xl font-bold ${getAccuracyColor(metrics["30_day"].accuracy)}`}
            >
              {(metrics["30_day"].accuracy * 100).toFixed(1)}%
            </div>
            <div class="text-xs text-slate-500 mt-2">
              {metrics["30_day"].correct_predictions} / {metrics["30_day"]
                .total_predictions} correct
            </div>
          </div>
        </div>
              ...
              <table class="w-full text-sm">
                <thead>
                  <tr class="border-b border-white/10">
                    ...
                    >
                      Model <span class="text-xs ml-1"
                        >{getSortIcon("model")}</span
                      >
                    </th>
                    <th
                    ...
                    >
                      Predictions <span class="text-xs ml-1"
                    ...
                    >
                      Correct <span class="text-xs ml-1"
                        >{getSortIcon("correct")}</span
                      >
                    </th>
                    <th
                    ...
                    >
                      Accuracy <span class="text-xs ml-1"
                        >{getSortIcon("accuracy")}</span
                      >
                    </th>
                  </tr>
                </thead>
                  ...
                  {#each getSortedModels(metrics.model_comparison) as [model, stats]}
                    ...
                    >
                      ...
                      <td class="py-3 px-4 text-right">
                        <span
                          class={`font-bold ${getAccuracyColor(stats.accuracy)}`}
                        >
        ...
        {#if livePredictions && livePredictions.length > 0}
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div class="glass-card p-4">
              ...
              <div
                class={`text-3xl font-bold ${completed.length > 0 ? getAccuracyColor(correct.length / completed.length) : "text-slate-500"}`}
              >
                {completed.length > 0
          ...
          <div class="glass-card p-6">
            ...
            <div class="overflow-x-auto">
              <table class="w-full text-sm">
                <thead>
                  <tr class="border-b border-white/10">
                    <th class="text-left py-3 px-4 font-medium text-slate-400"
                      >Match</th
                    >
                    <th class="text-center py-3 px-4 font-medium text-slate-400"
                      >League</th
                    >
                    <th class="text-center py-3 px-4 font-medium text-slate-400"
                      >Prediction</th
                    ...
                    <th class="text-center py-3 px-4 font-medium text-slate-400"
                      >Status</th
                    >
                    <th class="text-right py-3 px-4 font-medium text-slate-400"
                      >Date</th
                    >
                  </tr>
                </thead>
                    ...
                    >
                      ...
                      <td class="py-3 px-4">
                        <div class="flex items-center gap-2">
                          ...
                          >
                            <div
                              class="{conf.color} h-full rounded-full"
                      ...
                      <td class="py-3 px-4 text-center">
                        <span
                          class="px-2 py-1 rounded text-xs font-medium {status.color}"
                        >
                          {status.icon}
                          {status.label}
                        </span>
                      ...
                      <td class="py-3 px-4 text-right text-slate-400 text-xs">
                        {formatDate(
                          prediction.match_date || prediction.created_at,
                        )}
                      </td>
                    </tr>
                  {/each}
        ...
        {:else}
          <div class="glass-card p-8 text-center">
            ...
            <p class="text-slate-400 mb-4">
              Predictions will appear here as users request them through the
              app.
            </p>
            <p class="text-sm text-slate-500">
              Each prediction is logged to the database with full model
      ...
      {#if activeTab === "history"}
          ...
          {#if backtestSummary}
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div class="glass-card p-4">
                ...
                <div
                  class={`text-3xl font-bold ${getAccuracyColor(backtestSummary.avg_accuracy / 100)}`}
                >
              ...
              <div class="glass-card p-4">
                ...
                <div
                  class={`text-3xl font-bold ${getProfitColor(backtestSummary.total_profit)}`}
                >
                  ${backtestSummary.total_profit.toFixed(2)}
                </div>
                <div class="text-xs text-slate-500 mt-2">
                  Simulated betting profit
                </div>
              </div>
              ...
              <div class="glass-card p-4">
                ...
                {#if backtestSummary.best_week}
                  <div class="text-3xl font-bold text-emerald-400">
                    {backtestSummary.best_week.summary?.accuracy?.toFixed(1) ||
                      "N/A"}%
                  </div>
                  <div class="text-xs text-slate-500 mt-2">
                  ...
                  {#each [...backtestHistory].reverse() as week}
                    ...
                    >
                      <td class="py-3 px-4 font-medium"
                        >{formatDate(week.date)}</td
                      >
                      <td class="py-3 px-4 text-center text-slate-300"
                        >{week.summary?.evaluated || week.sample_size || 0}</td
                      >
                      <td class="py-3 px-4 text-center text-slate-300"
                        >{week.summary?.correct || 0}</td
                      >
                      <td class="py-3 px-4 text-center">
                        <span
                          class={`font-bold ${getAccuracyColor((week.summary?.accuracy || 0) / 100)}`}
                        >
                      ...
                      <td class="py-3 px-4 text-center">
                        <span
                          class={`font-medium ${getProfitColor(week.summary?.roi || 0)}`}
                        >
                      ...
                      <td class="py-3 px-4 text-right">
                        <span
                          class={`font-bold ${getProfitColor(week.summary?.profit || 0)}`}
                        >
            ...
            {#if backtestHistory.length >= 2}
                ...
                >
                  ...
                  <defs>
                    ...
                    >
                      <stop
                        offset="0%"
                        style="stop-color: rgb(52, 211, 153); stop-opacity: 0.3"
                      />
                      <stop
                        offset="100%"
                        style="stop-color: rgb(52, 211, 153); stop-opacity: 0.05"
                      />
                    </linearGradient>
                  </defs>
                  ...
                  <path
                    d="{generateChartPath(
                      ...
                      chartWidth,
                      chartHeight,
                    )} L {points[points.length - 1]?.x || 0},{chartHeight -
                      20} L 20,{chartHeight - 20} Z"
                  ...
                  {#each points as point, i}
                    ...
                    >
                      <title
                        >{formatDate(point.date)}: {point.value.toFixed(
                          1,
                        )}%</title
                      >
                    </circle>
              ...
              <div class="flex justify-between mt-4 text-xs text-slate-500">
                ...
                <span class="text-emerald-400"
                  >Last 12 weeks accuracy trend</span
                >
                <span>{formatDate(chartData[chartData.length - 1]?.date)}</span>
              </div>
            {:else}
              <div
                class="h-48 flex items-center justify-center bg-white/5 rounded-lg"
              >
                <div class="text-center text-slate-400">
                  <div class="text-4xl mb-2">📊</div>
                  <p>Need more data for trend chart</p>
                  <p class="text-xs text-slate-500 mt-1">
                    At least 2 weeks of backtest data required
                  </p>
                </div>
              </div>
            {/if}
        ...
        {:else}
          <div class="glass-card p-8 text-center">
            ...
            <p class="text-slate-400 mb-4">
              Backtest results will appear here after the weekly training
              workflow runs.
            </p>
            <p class="text-sm text-slate-500">
              The workflow runs automatically every Monday at 3 AM UTC, or can
              be triggered manually from GitHub Actions.
            </p>
          </div>
        {/if}
      {/if}
