<script>
  ...
  async function fetchH2H() {
    ...
    try {
      ...
      error = get(_)("headToHead.failedToLoad");
    } finally {
      loading = false;
    }
  }
  ...
  function getTeamWinsLabel(teamName) {
    return get(_)("headToHead.teamWins", { values: { team: teamName } });
  }

  function getLastMeetingsLabel(count) {
    return get(_)("headToHead.lastMeetings", { values: { count } });
  }
...
<div class="glass-card p-6 element-enter">
  ...
  {#if loading}
    <div class="text-center py-8">
      ...
      ></div>
      <p class="mt-2 text-sm text-slate-400">{$_('headToHead.loading')}</p>
    </div>
  {:else if error}
  ...
  {:else if h2hData}
    ...
    <div class="grid grid-cols-3 gap-4 mb-6">
      <div class="text-center">
        ...
        <div class="text-xs text-slate-400">{getTeamWinsLabel(homeTeam.name)}</div>
      </div>
      <div class="text-center">
        <div class="text-2xl font-bold text-yellow-400">{h2hData.draws}</div>
        <div class="text-xs text-slate-400">{$_('prediction.draw')}</div>
      </div>
      <div class="text-center">
        <div class="text-2xl font-bold text-red-400">{h2hData.away_wins}</div>
        <div class="text-xs text-slate-400">{getTeamWinsLabel(awayTeam.name)}</div>
      </div>
    </div>
    ...
    {#if h2hData.recent_matches && h2hData.recent_matches.length > 0}
      <div class="space-y-2">
        <div class="text-sm font-bold text-slate-400 mb-2">
          {getLastMeetingsLabel(h2hData.recent_matches.length)}
        </div>
