<script>
  ...
  function getOutcome(pred) {
    ...
    if (p.home_win_prob > p.away_win_prob && p.home_win_prob > p.draw_prob) {
      return { label: translate("prediction.homeWin"), color: "text-emerald-400" };
    } else if (p.away_win_prob > p.home_win_prob && p.away_win_prob > p.draw_prob) {
      return { label: translate("prediction.awayWin"), color: "text-rose-400" };
    }
    return { label: translate("prediction.draw"), color: "text-slate-400" };
  }
</script>
...
{#if compareFixtures.length > 0 && !isOpen}
  ...
  >
    <span>⚖️</span>
    <span class="font-bold">{compareButtonLabel}</span>
  </button>
{/if}
              ...
              <div class="glass-card p-4 relative {!predictions[index] && !loading[index] ? 'border-dashed border-2 border-white/20' : ''}">
                ...
                {:else if predictions[index]}
                  ...
                  <div class="flex items-center justify-between mb-4">
                    <div class="flex-1 text-center">
                      ...
                      <div class="text-sm font-medium truncate">
                        {pred.fixture_details?.teams?.home?.name || $_('compare.homeFallback')}
                      </div>
                    </div>
                    ...
                    <div class="flex-1 text-center">
                      ...
                      />
                      <div class="text-sm font-medium truncate">
                        {pred.fixture_details?.teams?.away?.name || $_('compare.awayFallback')}
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="text-center mb-4">
                    ...
                    <div class="text-xs text-slate-400 mt-1">{$_('prediction.predictedScore')}</div>
                  </div>
          ...
          {#if predictions[0] && predictions[1]}
            <div class="mt-6 glass-card p-4">
              ...
              <div class="grid grid-cols-3 gap-2 text-center text-sm">
                ...
                <div class="py-2 text-slate-400">{$_('prediction.confidence')}</div>
                ...
                <div class="py-2 text-slate-400">{$_('compare.homeGoals')}</div>
                <div class="py-2">
