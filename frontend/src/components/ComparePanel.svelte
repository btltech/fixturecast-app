...
{#if compareFixtures.length > 0 && !isOpen}
  ...
  >
    <span>⚖️</span>
    <span class="font-bold">{$_('compare.button', {values: {count: compareFixtures.length}})}</span>
  </button>
{/if}
      ...
      <div class="p-4">
        {#if compareFixtures.length === 0}
          <div class="text-center py-12 text-slate-400">
            ...
            <p>{$_('compare.emptyTitle')}</p>
            <p class="text-sm mt-2">{$_('compare.emptyDescription')}</p>
          </div>
        {:else}
          ...
          <div class="md:hidden space-y-3">
            <div class="flex gap-2">
              {#each [0, 1, 2] as tabIndex}
                ...
                >
                  {tabIndex === 0 ? (compareFixtures[0] ? $_('compare.fixture1') : $_('compare.fixture1empty')) :
                  tabIndex === 1 ? (compareFixtures[1] ? $_('compare.fixture2') : $_('compare.fixture2empty')) : $_('compare.summaryTab')}
                </button>
              {/each}
            </div>
                ...
                <div class="glass-card p-4 relative min-h-[320px] max-h-[520px] overflow-hidden">
                  ...
                  {:else if predictions[0]}
                    ...
                    <div class="flex flex-col h-full overflow-y-auto pr-1 gap-3">
                      ...
                      <div class="text-center">
                        ...
                        <div class="text-xs text-slate-400 mt-1">{$_('compare.predictedScore')}</div>
                      </div>
                      ...
                      <div class="pt-3 border-t border-white/10 grid grid-cols-2 gap-3 text-xs">
                        <div class="bg-white/5 rounded-lg p-2 text-center">
                          <div class="text-slate-400">{$_('prediction.btts')}</div>
                          <div class="font-bold">{((pred.prediction?.btts_prob || 0) * 100).toFixed(0)}%</div>
                        </div>
                        <div class="bg-white/5 rounded-lg p-2 text-center">
                          <div class="text-slate-400">{$_('prediction.over25')}</div>
                      ...
                      >
                        <span>🔮</span>
                        <span>{$_('compare.viewFullAnalysis')}</span>
                  ...
                  {:else}
                    <div class="text-center py-8 text-slate-400">
                      ...
                      <p class="text-sm">{$_('compare.addFixture')}</p>
                    </div>
                  {/if}
                </div>
              {:else}
                <div class="glass-card p-4 text-center text-slate-400">{$_('compare.addToSlot1')}</div>
              {/if}
            {:else if activeTab === 1}
              ...
              {:else}
                <div class="glass-card p-4 text-center text-slate-400">{$_('compare.addToSlot2')}</div>
              {/if}
            {:else}
              <div class="glass-card p-4">
                {#if predictions[0] && predictions[1]}
                  <h3 class="font-bold mb-3 flex items-center gap-2">
                    <span>📊</span> {$_('compare.quickComparison')}
                  </h3>
                  <div class="overflow-x-auto">
                    <table class="w-full text-sm">
                      <thead>
                        <tr class="text-slate-400 text-xs">
                          <th class="text-left py-2 font-medium">{$_('compare.fixture1')}</th>
                          <th class="text-center py-2 font-medium">Metric</th>
                          <th class="text-right py-2 font-medium">{$_('compare.fixture2')}</th>
                        </tr>
                      </thead>
                      <tbody>
                        <tr class="border-t border-white/5">
                          ...
                          <td class="py-2 text-center text-slate-400">{$_('compare.confidence')}</td>
                        ...
                        <tr class="border-t border-white/5">
                          ...
                          <td class="py-2 text-center text-slate-400">{$_('compare.score')}</td>
                        ...
                        <tr class="border-t border-white/5">
                          ...
                          <td class="py-2 text-center text-slate-400">{$_('prediction.btts')}</td>
                        ...
                        <tr class="border-t border-white/5">
                          ...
                          <td class="py-2 text-center text-slate-400">{$_('prediction.over25')}</td>
